...
export const card = (() => {
    ...
    return `
        <button style="font-size: 0.8rem;" onclick="undangan.comment.like.love(this)" data-uuid="${
          c.uuid
            ...
            <span class="my-0 mx-1" data-count-like="${c.like.love}">${
      c.like.love
    }</span>
            <i class="me-1 ${
              likes.has(c.uuid)
  ...
  const renderAction = (c) => {
    ...
    if (session.isAdmin() && c.is_admin && (!c.gif_url || gif.isActive())) {
      action += `<button style="font-size: 0.8rem;" onclick="undangan.comment.edit(this, ${
        c.is_parent ? "true" : "false"
      })" data-uuid="${
        c.uuid
      }" class="btn btn-sm btn-outline-auto rounded-4 py-0 me-1 shadow-sm" data-own="${
        c.own
      }" data-offline-disabled="false">Edit</button>`;
    } else if (
      owns.has(c.uuid) &&
      ...
      action += `<button style="font-size: 0.8rem;" onclick="undangan.comment.edit(this, ${
        ...
        c.uuid
      }" class="btn btn-sm btn-outline-auto rounded-4 py-0 me-1 shadow-sm" data-offline-disabled="false">Edit</button>`;
    }
  ...
  const renderReadMore = (uuid, uuids) => {
    ...
    return `<a class="text-theme-auto" style="font-size: 0.8rem;" onclick="undangan.comment.showOrHide(this)" data-uuid="${uuid}" data-uuids="${util.escapeHtml(
      uuids.join(",")
    )}" data-show="${
      hasId ? "true" : "false"
    }" role="button" class="me-auto ms-1 py-0">${
      hasId ? "Hide replies" : `Show replies (${uuids.length})`
    }</a>`;
  };
  ...
  const renderTracker = (c) => {
    ...
    return `
        <div class="mb-1 mt-3">
            <p class="text-theme-auto mb-1 mx-0 mt-0 p-0" style="font-size: 0.7rem;" id="ip-${
              c.uuid
            }"><i class="fa-solid fa-location-dot me-1"></i>${util.escapeHtml(
      c.ip
    )} <span class="mb-1 placeholder col-2 rounded-3"></span></p>
            <p class="text-theme-auto m-0 p-0" style="font-size: 0.7rem;"><i class="fa-solid fa-mobile-screen-button me-1"></i>${util.parseUserAgent(
              util.escapeHtml(c.user_agent)
            )}</p>
        </div>`;
  };
  ...
  const renderHeader = (c) => {
    ...
    return `class="${
      !showHide.get("hidden").find((i) => i.uuid === c.uuid)["show"]
        ? "d-none"
        : ""
  ...
  const renderBody = async (c) => {
      ...
      return (
        ...
        `
            <div class="d-flex justify-content-center align-items-center my-2">
                <img src="${await gif.get(c.gif_url)}" id="img-gif-${
          c.uuid
        }" class="img-fluid mx-auto gif-image rounded-4" alt="selected-gif">
            </div>`
      );
    }
    ...
    return (
      head +
      `
        <p class="text-theme-auto my-1 mx-0 p-0" style="white-space: pre-wrap !important; font-size: 0.95rem;" data-comment="${util.base64Encode(
          c.comment
  ...
  const renderContent = async (c) => {
    ...
    const resData = await Promise.all(
      c.comments.map((cmt) => renderContent(cmt))
    );
    ...
    const template = `
        ...
        <div class="d-block mb-2" id="comment-form-${id}">
            <div class="position-relative">
                ...
                <textarea class="form-control shadow-sm rounded-4 mb-2" id="form-inner-${id}" minlength="1" maxlength="1000" placeholder="Tulis balasan komentar" rows="3" data-offline-disabled="false"></textarea>
            </div>
        </div>
  ...
  const renderEdit = (id, presence, is_parent, is_gif) => {
    ...
    const template = `
        ...
        ${
          !is_gif
            ? `<textarea class="form-control shadow-sm rounded-4 mb-2" id="form-inner-${id}" minlength="1" maxlength="1000" placeholder="Tulis perubahan komentar" rows="3" data-offline-disabled="false"></textarea>    
        `
            : `${
                !gif.isActive()
                  ? ""
